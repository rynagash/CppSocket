...
CServer::CServer(int _port, int _bs) {
    ready = false;
    port = _port;
    bufferSize = _bs;
...
int CServer::receive(char* buf, int len) {
    int revd_size;
    int tmp;
    revd_size = 0;
    while (revd_size < len) {
...
void CServer::sendStr(const std::string& str) {
    ready = false;
    send(dstSocket, str.c_str(), str.length(), 0);
}
void CServer::sendData(char *buf, int size) {
    ready = false;
